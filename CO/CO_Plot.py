...
# ── File Paths ────────────────────────────────────────────────────────
nc_file        = r"C:\Users\samue\Documents\Conferences\Space\CO\CO_Data.nc"
...
co  = prod.variables['carbonmonoxide_total_column'][0, :, :].flatten()
...
grid_co = griddata((lon_filt, lat_filt), co_filt,
                   (grid_lon, grid_lat), method='nearest')

# ── Mask Outside Nepal  (dy NEGATIVE = north-to-south) ───────────────
dx        = (bounds[2] - bounds[0]) / RES
dy        = (bounds[3] - bounds[1]) / RES          # positive step magnitude
transform = Affine.translation(bounds[0], bounds[3]) * Affine.scale(dx, -dy)

mask_arr = features.geometry_mask(
    nepal.geometry, out_shape=grid_co.shape,
    transform=transform, invert=False              # True = outside Nepal
)
grid_co[mask_arr] = np.nan
...
vmin = np.nanpercentile(grid_co, 2)
vmax = np.nanpercentile(grid_co, 98)

im = ax.pcolormesh(grid_lon, grid_lat, grid_co,
...
cbar.set_label(r'CO Column (mol m$^{-2}$)', fontsize=9, labelpad=8)
cbar.ax.tick_params(labelsize=8)

ax.set_title('Carbon Monoxide (CO) Concentration', fontsize=12,
             fontweight='bold', pad=10)

plt.savefig('CO_Plot.png', dpi=300, bbox_inches='tight', pad_inches=0.15)
plt.show()
print("Saved → CO_Plot.png")

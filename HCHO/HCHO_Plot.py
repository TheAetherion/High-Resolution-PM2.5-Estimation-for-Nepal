...
# ── File Paths ────────────────────────────────────────────────────────
nc_file        = r"C:\Users\samue\Documents\Conferences\Space\HCHO\HCHO_Data.nc"
...
# Standard TROPOMI HCHO variable (tropospheric vertical column)
data_var = prod.variables['formaldehyde_tropospheric_vertical_column'][0, :, :].flatten()
qa       = prod.variables['qa_value'][0, :, :].flatten()

# QA > 0.5 is the standard science-quality filter for HCHO
...
# 'YlGn' suits biogenic/vegetation-linked HCHO emissions
...
cbar.set_label(r'HCHO Column (mol m$^{-2}$)', fontsize=9, labelpad=8)
cbar.ax.tick_params(labelsize=8)

ax.set_title('Formaldehyde (HCHO) Concentration', fontsize=12,
             fontweight='bold', pad=10)

plt.savefig('HCHO_Plot.png', dpi=300, bbox_inches='tight', pad_inches=0.15)
plt.show()
print("Saved → HCHO_Plot.png")
